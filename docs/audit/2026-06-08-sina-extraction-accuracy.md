...
## 3. 准确率指标 (200 stocks × 3 years × 3 statements = 1,769 对比)

| 报表 | 匹配率 | 值准确率 | RDS 字段总数 | 匹配字段数 | 对比数 |
|------|--------|----------|--------------|------------|--------|
| 资产负债表 (BS) | **99.45%** | **95.52%** | 26,433 | 26,287 | 586 |
| 利润表 (IS) | **92.26%** | **62.04%** | 17,091 | 15,769 | 593 |
| 现金流量表 (CF) | **79.93%** | **69.00%** | 16,021 | 12,806 | 590 |

**匹配率** = Sina 有对应 RDS 字段的数量 / RDS 总字段数
**值准确率** = 匹配字段中值差异 <10% 的比例

**IS 值准确率分析** (P1-1 修复后):
- 非银行: 96.4-96.7% (优秀)
- 银行: 37.5-89.3% (改进中，从 11.1% 提升)
- 整体: 62.04% (受银行拖累)

**CF 匹配率分析**:
- CF 匹配率 79.93% 较低，因为 Sina 只有直接法数据，RDS 包含间接法调节项
- 值准确率 69.00% 表明匹配上的字段值差异较大
...
| 限制 | 影响 | 状态 |
|------|------|------|
| IS 62.10% 值准确率 | 银行 IS 匹配策略仍需优化 | P1-1 部分修复 (11%→37.5%) |
| F057N 信用减值损失 | 银行 IS 正确输出 F057N | ✅ P1-7 已修复 |
| 2022 基线数据 | RDS 只有 Q1 数据，无年报 | ✅ P0-4 已标注 |
| CF 间接法调节项 | Sina 不提供，已过滤 | 设计约束 |
| test_word_recovery | PDF 数据缺失 | ✅ P0-2a 已跳过 |
