...
from astock_fundamentals.sources.pdf.parsers.table_parser import TableParser
...
class BaseExtractor(ABC):
    ...
    def _do_extract(self, parser: PdfParser, discovered_pages: List[int] = None) -> Dict:
        ...
        if found_items < min_items:
            from astock_fundamentals.core.semantic_recovery import SemanticRecovery
            import os as _os
            ...
            if pdf_path and _os.path.exists(pdf_path):
                ...
                if len(result["data"]) < min_items:
                    from astock_fundamentals.core.word_recovery import recover_statement_auto
